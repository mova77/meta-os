...
Every script and hook that automates the <SPACE> process, with what it enforces and how
...
| `scripts/prune_ghost_sprints.py` | Prune **ghost sprints** — empty sprints lingering in Jira's Agile API but invisible in the board UI (e.g. duplicate `<SPACE> Sprint 8`/id 565), which pollute `futureSprints()`/`openSprints()` JQL. Skips ACTIVE and non-empty sprints. Same `JIRA_EMAIL`/`JIRA_API_TOKEN` auth as `sync.py`. | Read-only dry-run; deletes only with `--delete` (+ `--yes`) |
...
| `validate-backlog.sh` | PostToolUse·Edit\|Write | On a `scrum/<space>/backlog.json` edit: hard-fails only on invalid JSON; flags duplicate/unknown `jiraId`, non-canonical status, and unresolved `<SPACE>-` refs as advisories to reconcile via `sync.py`. Accepts `NO GO`. | — (advisory) |
...
| `provision-jdk.sh` | SessionStart | Auto-provisions **JDK 25** in the remote/web env (<SPACE>-622) so the build toolchain matches. | — |
